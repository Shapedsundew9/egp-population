...
_DEFAULT_POPULATION_METRICS_CONFIG: TableConfigNorm = (
    raw_table_config_validator.normalized(
        {
            "database": {"dbname": "erasmus"},
...
def configure_populations(
    ...
    """
    p_table_config: TableConfigNorm = raw_table_config_validator.normalized(
        table_config
    )
    ...
    defined_uids: dict[int, PopulationConfig] = {
        config["uid"]: config for config in pcs if "uid" in config
    }
    for defined_config in p_table.select(
        _POPULATION_UID_EXISTS_SQL, {"uids": list(defined_uids.keys())}
    ):
    ...
    for config in populations_config:
        if not population_entry_validator.validate(config):
            assert (
                False
            ), f"Population configuration is not valid:\n{population_entry_validator.error_str()}"
    ...
    configs: tuple[PopulationConfig, ...] = tuple(
        population_entry_validator.normalized(p) for p in populations_config
    )
    hashes: list[bytes] = [
        p["population_hash"] for p in configs if "population_hash" in p
    ]
    existing: set[bytes] = set(
        p_table.select(
            _POPULATION_CONFIG_EXITS_SQL,
            {"hashes": hashes},
            ("population_hash",),
            "tuple",
        )
    ...
    p_table.insert(
        (
            ...
            if config["population_hash"] in set(hashes) - existing
        )
    )
    p_configs: dict[int, PopulationConfig] = {
        c["uid"]: c for c in p_table.select(_POPULATION_CONFIGS_SQL, {"hashes": hashes})
    }
    ...
    for config in p_configs.values():
        _logger.info(
            f"Loading population name: {config.get('Name')}, UID: {config.get('uid')}, hash: {config.get('population_hash')}"
        )
        if config.get("git_repo") is not None:  # Then all git fields are not None.
            ...
            if not isdir(git_path):
                ...
                makedirs(git_path)
            _logger.info(
                f"Population definition in git repo '{git_repo}' at commit {git_hash}"
            )
            ...
            if isdir(config.get("git_repo", "__invalid_dir__")):
                _logger.info(
                    f"Git repo folder '{git_repo}' exists. Using current checkout."
                )
                chdir(config.get("git_repo", "."))
                result: CompletedProcess[bytes] = run(
                    ["git", "rev-parse", "--verify", "HEAD"], stdout=PIPE, check=False
                )
                log_level: Callable[..., None] = (
                    _logger.error if not result.returncode else _logger.info
                )
                log_level("\n" + result.stdout.decode("utf-8"))  # pylint: disable=W1201
                assert result.returncode == 0, (
                    f"Is {git_repo} a valid git repo? ",
                    "'git rev-parse --verify HEAD' produced a non-zero return code.",
                )
                hash_str: str = result.stdout.decode("utf-8")
                assert (
                    len(hash_str) == 41 or len(hash_str) == 65
                ), "Unexpected length of returned hash '{hash_str}'."
                hash_str = hash_str[:-1]
                if populations_config.get("error_on_commit_hash_mismatch", True):
                    assert (
                        hash_str == git_hash
                    ), f"Git has is '{hash_str}' was expecting '{git_hash}'."
                    ...
                    _logger.info("Git checkout hash matches expectations.")
                _logger.info(
                    "Not installing requirements of population git repo as repo already exists."
                )
            else:
                # Clone the repo
                url: str = config.get("git_url", "") + "/" + git_repo + ".git"
                result: CompletedProcess[bytes] = run(
                    ["git", "clone", url], stdout=PIPE, check=False
                )
                log_level: Callable[..., None] = (
                    _logger.error if not result.returncode else _logger.info
                )
                ...
                log_level("\n" + result.stdout.decode("utf-8"))  # pylint: disable=W1201
                assert result.returncode == 0, (
                    f"Is {git_hash} a valid commit hash for this repo? ",
                ...
                if isfile("requirements.txt"):
                    ...
                    assert result.returncode == 0, (
                        "Non-zero return code trying to install python requirements. ",
                        "'pip install -r requirements.txt'",
                    )
                else:
                    _logger.info(
                        f"No requirements.txt file exists for population {config.get('uid')}."
                    )
            chdir(cwd)

        # Import the preload function
        _logger.info(
            f"Importing the preload function for population {config.get('uid')}."
        )
        ...
        _logger.info(
            f"Importing the fitness function for population {config.get('uid')}."
        )
        ...
        _logger.info(
            f"Importing the survivability function for population {config.get('uid')}."
        )
        survivability_import: str = config.get(
            "survivability_import", "__configuration_error__"
        )
        import_str = f"from {survivability_import} import survivability_function as survivability_function_{config.get('uid')}\n"
        exec(import_str, globals())
        exec(
            f"config['survivability_function'] = survivability_function_{config.get('uid')}"
        )
...
def new_population(
    population_config: PopulationConfigNorm, gpool: gene_pool, num: int | None = None
) -> None:
    ...
    num_to_create: int = (
        num if num is not None else population_config.get("active_size", 100)
    )
    _logger.info(
        f"Creating {num_to_create} GC's for population index: {population_config['uid']}"
    )
    ...
    for _ in range(num_to_create):
        ...
        _logger.debug(
            f'Created GGCs to add to Gene Pool: {[ref_str(ggc["ref"]) for ggc in (rgc, *fgc_dict.values())]}'
        )
